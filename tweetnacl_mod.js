(function (nacl) {
  ...
  //changes (hash function to blake2b) based on what coranos did in bananojs
  ...
  const crypto_hash_blake2b = (input, size) => blake2b(size).update(input).digest();
  ...
  function crypto_sign(sm, m, n, sk) {
    ...
    r = crypto_hash_blake2b(sm.subarray(32), 64);
    reduce(r);
    scalarbase(p, r);
    pack(sm, p);

    for (i = 32; i < 64; i++) sm[i] = pk[i - 32]; //sk[i] to pk[i - 32]
    h = crypto_hash_blake2b(sm, 64);
    reduce(h);
  ...
  var crypto_secretbox_KEYBYTES = 32,
    ...
    //!!! BANANO CHANGE: we are replacing the below line with 32 instead of 64
